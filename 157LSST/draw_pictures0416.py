...
record_root = r"/mnt/nodestor/home/sincloud_user/Cunshi/Tingyu/LSST/DATA/Record/LSTest"
...
def process_file(file_path,output_subfolder):
    try:
        ...
        for band in existing_bands:
            ...
            time_diffs = np.diff(np.sort(time_seconds)) + 30
            ...
            max_freq = min(1/(0.5*median_diff), 1e4)  # Cap at 10kHz (adjust per your needs)
            min_freq = max(1/(10*total_time), 1e-7)  # Minimum 0.1μHz
