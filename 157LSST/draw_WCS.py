...
def process_file(file_path, output_subfolder):
    try:
        ...
        df["FLUXCAL"] = pd.to_numeric(df["FLUXCAL"], errors='coerce')
        df["FLUXCALERR"] = pd.to_numeric(df["FLUXCALERR"], errors='coerce')
        df = df.dropna(subset=["MJD", "FLUXCAL", "FLUXCALERR"], how='any')  # Drop rows with NaN

        # Ensure flux errors are positive and non-zero
        df = df[df["FLUXCALERR"] > 0]
        ...
        file_id = os.path.splitext(os.path.basename(file_path))[0]
        print(file_id)  # Output: "photo"
        '''
        file_output_folder = os.path.join(output_subfolder, file_id)
        os.makedirs(file_output_folder, exist_ok=True)

        for band in existing_bands:
            band_data = filtered_df[(filtered_df["BAND"] == band) & (filtered_df["detect"] == 1)]
            if band_data.empty:
                continue
            ...
            try:
                ...
                plt.plot(1 / frequency, power, color=band_colors.get(band, "gray"))
